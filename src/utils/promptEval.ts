 ...
 *  koreatech 프로젝트활동 STEP_KEYWORDS를 단계별로 병합해 이식. */
export const PBL_STAGE_KEYWORDS: Record<string, string[]> = {
  recognition: ['불편|번거|문제|아쉬', '누가|누구|사용자', '언제|상황|때', '후보|선정|선택|고르', '기준|자주|데이터|가능', '요약|한 문장|한문장'],
  definition: ['돕|해결|위해', '사용자|대상|학생', '목표|가치|이득|절약', '성공|기준|완료', '표시|출력|보이|결과', '포함', '제외|뺀|안함'],
  decomposition: ['입력', '처리|계산|판단|정렬', '출력|결과', '기능|하기|하위', '나누|쪼개|분해', '→|연결|흐름|이어', '핵심|선택', '난이도|우선|먼저|순서'],
  abstraction: ['핵심|필요', '제거|버리|빼|무시', '리스트|딕셔너리|구조|배열|list|dict', '값|예시|항목', '규칙|공식|=', '모든|항상|일반', '반복|패턴|for|루프', '같은|비슷'],
  algorithm: ['순서|단계|1.|①', '입력|처리|출력', '의사코드', '만약|if|조건|이면', '반복|for|while|루프', '순서도|검증|확인|테스트', '예시|결과|일치'],
  implementation: ['함수|def|코드', '리스트|딕셔너리|변수|구조', '실행|테스트|print|출력', '기대|결과|확인', '오류|에러|버그|디버그', '수정|고치|개선', '핵심|먼저|기능'],
  presentation: ['문제', '해결|아이디어|과정', '시연|발표|데모', '배운|개선|회고', '링크|github|배포'],
};
