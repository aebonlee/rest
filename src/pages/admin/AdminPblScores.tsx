        ...
        <div className="admin-content">
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', flexWrap: 'wrap', gap: '8px', marginBottom: '8px' }}>
            ...
            <span style={{ fontSize: '13px', color: 'var(--text-secondary, #6b7280)' }}>
              제출 {rows.length}명 · 단계별 강사 점수(괄호는 자동환산) · 개별 채점은 <a href="/pbl/eval" style={{ color: 'var(--primary-blue, #0046C8)' }}>PBL 평가</a>
            </span>
          </div>

          {/* 도구 + 단계 평균을 한 줄에 — 정렬/다운로드 좌, 평균 칩 우 (여백 최소화) */}
          ...
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '40px 20px', background: 'var(--bg-light-gray, #f8f9fa)', borderRadius: '12px', color: 'var(--text-secondary, #6b7280)' }}>
                ...
                <tbody>
                  {sorted.map((r, idx) => (
                    <tr key={r.user_id}>
                      ...
                      <td style={{ textAlign: 'center', padding: '5px 2px', fontWeight: 800, color: scoreColor(totalScore(r.scores), PBL_TOTAL) }}>
                        {totalScore(r.scores)}
                      </td>
                      <td style={{ textAlign: 'center', padding: '5px 2px', fontWeight: 700, color: '#94a3b8' }}>{autoTotal(r.auto)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
