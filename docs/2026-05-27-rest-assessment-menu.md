...
## 14. 후속 작업: 코드 블록 복사 버튼 + 학습노트 4시간 강의 깊이 확장 (2026-05-27 동일자)

### 14-1. 코드 블록 복사 버튼 (커밋 `2531e02`)

#### 배경
사용자 요청: "학습내용의 소스코드 위에 '복사' 버튼 만들어줘"

학습자가 코드 예제를 직접 입력하지 않고 즉시 실습에 활용할 수 있도록 모든 코드 블록에 복사 기능을 추가.

#### 구현 내용
- `CodeBlock` 컴포넌트 신설 — 헤더(언어 라벨 + 복사 버튼) + 코드 영역 분리
- `navigator.clipboard.writeText` 우선 사용, 권한 부재 시 `execCommand` 폴백
- 복사 성공 시 1.5초간 녹색 체크 아이콘 + "복사됨" 상태
- 외부 의존성 없이 인라인 SVG 아이콘 사용

#### 파일 변경
| 파일 | 변경 |
|------|------|
| `src/pages/Learning.tsx` | CodeBlock 컴포넌트 추가, 기존 `<pre>` 직접 렌더링을 컴포넌트 호출로 교체 (+94/-23) |

---

### 14-2. 학습노트 4시간 강의 깊이 확장 (커밋 `851da89`)

#### 배경
사용자 요청: "1일 4시간의 강의야. 강의자료 따로 만들지 않고 웹에서의 자료로 수업 진행 할 수 있을 만큼으로 각각의 날짜별 강의내용의 학습 노트를 더 상세하고 기술적으로 제작해줘"

13장의 콘텐츠 보강은 학습노트 수준은 도달했으나 **별도 슬라이드 없이 4시간 강의 진행이 가능한 교재 수준**에는 미달. 17개 일자 전체를 강의 진행 가능 수준으로 재확장.

#### 모든 17일에 일관 적용한 6개 신규 섹션
| 섹션 | 형식 | 용도 |
|------|------|------|
| 강의 흐름 | 4행 표 (S1~S4) | 50분×4세션 시간 배분 + 세션별 산출물 |
| FAQ | 5개 Q&A | 실제 강의에서 자주 받는 질문 |
| 자가 점검 퀴즈 | 5문제 + 정답 콜아웃 | 학습 직후 자가 평가 |
| 참고 자료 | 5개 링크 | 공식 docs·도서·강의 |
| 다음 시간 예고 | 단락 | 연속 학습 흐름 강화 |
| 일자별 특화 | 코드/표 | 일자 주제에 맞는 심화 자료 |
...
| Day | 추가 콘텐츠 |
|-----|-------------|
| 1 | AI 60년 역사 변곡점 표 (튜링→ChatGPT) / 산업별 활용 사례 6분야 |
| 2 | 역할극(Role-Play) 프롬프트 예시 / 5차원 평가 매트릭스 |
| 3 | 폴백(Fallback) 패턴 TypeScript 코드 / 응답 캐싱 메모리 캐시 코드 |
| 4 | VS Code 단축키 15개 (Win/Mac 병기) / Git 충돌 해결 워크플로우 / npm 핵심 명령 / 첫 GitHub Pages 배포 단계별 / 선수과정 마무리 점검 7개 |

**정규과정**
| Day | 추가 콘텐츠 |
|-----|-------------|
| 1 | Cursor 단축키 8개 / AI 검수 5단계 워크플로우 / 바이브코딩 학습 곡선 4주차 |
| 2 | CSS 선택자 우선순위 점수표 |
| 3 | JS falsy 7종 + optional chaining + nullish coalescing |
| 4 | React 렌더링 사이클 텍스트 도식 (Mount/Update/Unmount) |
| 5 | (FAQ + 퀴즈 + 참고만 추가, 기존 콘텐츠 충분) |
| 6 | (FAQ + 퀴즈 + 참고만 추가) |
| 7 | Crazy 8s 브레인스토밍 기법 단계별 / 아키텍처 도식 도구 4종 |
| 8 | (FAQ + 퀴즈 + 참고만 추가) |
| 9 | (FAQ + 퀴즈 + 참고만 추가) |
| 10 | (FAQ + 퀴즈 + 참고만 추가) |
| 11 | 과학적 디버깅 5단계 멘탈 모델 |
| 12 | SPA 404 트릭 (GitHub Pages용 public/404.html + index.html 스크립트) |
| 13 | 강의 마무리 멘트 + 운영사 메시지 콜아웃 |

#### 파일 변경
| 파일 | 변경 |
|------|------|
| `src/data/learningData.ts` | 17일 전체 콘텐츠 재확장 (+1,106 / -24) |

### 14-3. 청크 크기 변화 추적 (콘텐츠 보강의 정량적 증거)

| 시점 | Learning 청크 | gzip |
|------|---------------|------|
| 학습평가 도입 시 (`22ab445`) | ~20 kB | ~8 kB |
| 1차 콘텐츠 보강 (`588243b`) | 85.46 kB | 33 kB |
| 코드 블록 복사 버튼 (`2531e02`) | 87.13 kB | 33 kB |
| 4시간 강의 깊이 확장 (`851da89`) | **146.65 kB** | **54 kB** |

→ 콘텐츠 양 기준 약 **7배 증가**. 코드 분할로 초기 번들에는 영향 없음.

### 14-4. 빌드 & 배포
- `tsc -b --noEmit` 0 error
- `npm run build` 성공 (3.27s)
- `npx gh-pages -d dist` Published ✓ (두 차례)

### 14-5. 검증 체크리스트
- [x] 모든 17일에 강의 흐름 50분×4세션 표가 보임
- [x] FAQ·자가 점검·참고 자료·다음 시간 예고가 일관되게 표시됨
- [x] 정답 콜아웃이 tip 스타일로 강조되어 표시
- [x] 코드 블록에 복사 버튼이 정상 동작 (HTTPS·HTTP 양쪽)
- [x] 복사 시 1.5초간 "복사됨" 피드백
- [x] 다크모드에서 코드/표/콜아웃·복사 버튼 모두 가독성 유지

### 14-6. 강의 운영 가이드 (강사용 메모)
- 각 일자 시작 시 "강의 흐름" 표를 학습자에게 먼저 안내 — 시간 배분 인지
- 핵심 개념 설명 후 코드 블록 → 복사 버튼으로 즉시 학습자에게 실습 진입 유도
- 세션 종료 5분 전 자가 점검 퀴즈로 이해도 측정 → 다음 세션에 즉시 보강
- 4시간 후 FAQ 섹션을 활용해 미해결 질문 정리
- 참고 자료는 가정 학습/심화 학습용으로 강의 종료 시 안내

### 14-7. 잠재적 후속 작업
- 일자별 학습 진도 체크 (Supabase rest_progress 테이블)
- 자가 점검 퀴즈를 인터랙티브 객관식으로 전환 (정답 즉시 검증)
- 표·코드 PDF 출력 기능 (강의 노트 인쇄용)
- 강사용 발표자 모드 (한 화면에 메모 + 학습자 화면)

### 14-8. 커밋 이력 (2026-05-27 추가분)
```
2531e02  feat(learning): 코드 블록에 복사 버튼 추가
851da89  feat(learning): 4시간 강의 진행 가능한 깊이로 17일 전체 콘텐츠 확장
```
