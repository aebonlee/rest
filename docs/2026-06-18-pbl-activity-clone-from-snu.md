...
## 4차 수정 (2026-06-18) — 예시를 “한국형 AI 동화책 제작 앱”으로 교체 + 제출 링크 + CRUD

### 배경
- 예시 프로젝트를 **한국형 AI 동화책 제작 앱**(한국적 소재·아이 나이·주제를 입력하면 맞춤형 동화책을
  생성하는 앱)으로 교체. 예시는 참고용이며, 학생들은 각자 정한 23개 주제로 개별 진행.
- 발표는 패들렛으로 자료를 집계하되, 학생이 **사용한 패들렛 링크 + GitHub 링크**를 제출.

### 변경
- `config/pblActivity.ts`
  - 공통 예시 `EXAMPLE`을 “한국형 AI 동화책 제작 앱”으로 변경하고, 7단계 27개 작성 항목의 `example`(및
    관련 guide)을 동화책 앱 맥락으로 전면 교체(입력=소재·나이·주제, 처리=프롬프트→이야기 생성→장면 분할
    →삽화 생성, 출력=글+그림 동화책 등).
  - 발표 단계에 **`submit` 항목 추가** — “제출 링크 (패들렛 · GitHub)”. 본인이 사용한 패들렛 글 링크와
    GitHub/배포 링크를 붙여 제출. (발표 단계 = 발표 스크립트·시연·제출 링크·회고 4개 항목)
- `pages/pbl/PblStage.tsx` — **CRUD 보강**: 저장 버튼이 기존 저장 시 “수정 저장”으로 표기(Update),
  “작성 지우기” 버튼 추가(해당 단계 내용 삭제 후 저장 = Delete). 읽기·생성은 기존대로.
- `pages/pbl/PblInfo.tsx` — 예시는 참고용·각자 주제로 작성·언제든 수정 가능 안내 문구 추가.
- 강사/관리자 개인별 열람: 기존 `/pbl/eval`이 단계별 모든 작성 항목을 표시하므로 패들렛·GitHub 링크 포함
  학생별 처리 내역을 그대로 확인 가능.

---

## 5차 수정 (2026-06-18) — 학번 제거 + 기본정보에 프로젝트 주제 저장

### 변경
- `pages/pbl/PblInfo.tsx` — **학번(student_no) 입력 항목 제거**, **프로젝트 주제(project_topic) 입력 추가**.
  기본정보 = 이름·연락처·이메일·프로젝트 주제. (학번 필수 검증·프로필 학번 반영 제거)
- `utils/pblStore.ts` — `PblInfo`/`PblSubmission`에 `project_topic` 추가, `saveInfo`가 `project_topic` 저장.
  student_no는 optional로 남겨 하위 호환 유지(미사용).
- `pages/pbl/PblEval.tsx` — 강사 화면 학생 목록·헤더에서 학번 대신 **📌 프로젝트 주제** 표시.
- `supabase/2026-06-18-pbl-submissions.sql` — `project_topic TEXT` 컬럼 추가(CREATE + 멱등 ALTER).
  (이미 테이블을 만든 경우 ALTER 한 줄만 다시 실행하면 됨)
