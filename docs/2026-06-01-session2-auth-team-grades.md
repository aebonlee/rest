...
## 7. 동일인 통합 — 한 사람이 이메일 2개로 가입한 경우 (`utils/people.ts` + 관리자 4화면)
- **발견**: rest 가입자 중 **전화번호 동일 = 동일인**인 중복 2건
  - 임종권: ssujklim@gmail.com / deathbed0104@gmail.com (010-4052-0834)
  - 최재영: istp0109318@gmail.com / fghjkkzxvvbn@naver.com (010-9318-3978)
- **방침**(운영자 선택): 비파괴 — **두 이메일 모두 로그인 유지**, 관리 화면에서만 동일인으로 묶어 표시
- **공용 유틸 `groupByPerson`**: 전화번호(숫자만) 우선, 없으면 이름으로 그룹화. `PersonGroup{primary, accounts, ids[], emails[], isMerged}`
- **적용 화면**:
  - `AdminStudents`(수강생): 동일인 1행, 이메일 2개 표시, "동일인 N계정" 뱃지, 검색은 모든 이메일 대상, "계정 N개" 표기
  - `AdminGrades`(성적): 동일인의 두 계정 성적 중 **최고점 합산**, 통계도 인원 기준
  - `AdminAttendance`(출결): 동일인의 모든 계정 id로 출결 조회/집계, 신규 기록은 대표 계정에 저장, 월매트릭스 우선순위(출>지>사>결)
  - `AdminDashboard`: 수강생 수를 동일인 기준으로 카운트
  - `AdminRoster`(명단대조): 동일인을 한 명으로 매칭(중복을 "명단외 가입"으로 잘못 분류하던 버그 해결), 두 이메일 표시
- 계정 병합/삭제는 하지 않음 → 학생은 두 이메일 중 무엇으로 로그인해도 동일하게 관리됨
